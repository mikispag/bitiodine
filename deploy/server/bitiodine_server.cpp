...
int main()
{
    ...
    if (sqlite3_open("../blockchain/blockchain.sqlite", &db) == SQLITE_OK) {
        // Prepare queries
        sqlite3_prepare_v2(db, "SELECT MAX(tx_id) FROM tx", -1, &max_tx_id_stmt, NULL);
        sqlite3_prepare_v2(db, "SELECT txout.address FROM txin LEFT JOIN txout ON (txout.txout_id = txin.txout_id) WHERE txin.tx_id = ?", -1, &address_stmt, NULL);
        sqlite3_prepare_v2(db, "SELECT blocks.time, tx.tx_hash FROM tx LEFT JOIN blocks ON (tx.block_id = blocks.block_id) WHERE tx.tx_id = ?", -1, &details_stmt, NULL);
        sqlite3_prepare_v2(db, "SELECT txout.address, txout.txout_value FROM txout LEFT JOIN txin ON (txin.txout_id = txout.txout_id) WHERE txout.tx_id = ?", -1, &out_stmt, NULL);
        ...
        for (unsigned int tx_id = 1; tx_id <= max_tx_id; tx_id++) {
            ...
            if (sqlite3_bind_int(address_stmt, 1, (int) tx_id) != SQLITE_OK) {
                cerr << "Error: unable to bind tx_id = " << tx_id << " to address_stmt." << endl;
                return 1;
            }
            if (sqlite3_bind_int(details_stmt, 1, (int) tx_id) != SQLITE_OK) {
                cerr << "Error: unable to bind tx_id = " << tx_id << " to details_stmt." << endl;
                return 1;
            }
            if (sqlite3_bind_int(out_stmt, 1, (int) tx_id) != SQLITE_OK) {
                cerr << "Error: unable to bind tx_id = " << tx_id << " to out_stmt." << endl;
                return 1;
            }
            ...
            while (address_status == SQLITE_ROW) {
                ...
                if (sqlite3_step(details_stmt) == SQLITE_ROW) {
                    unsigned int timestamp = (unsigned int) sqlite3_column_int64(details_stmt, 0);
                    string tx_hash = string(reinterpret_cast<const char*>(sqlite3_column_text(details_stmt, 1)));
                ...
                while (sqlite3_step(out_stmt) == SQLITE_ROW) {
                    string out_address = string(reinterpret_cast<const char*>(sqlite3_column_text(out_stmt, 0)));
...
void print_cluster_label(int client, unsigned int cluster)
{
    ...
    if (sqlite3_open("../clusterizer/cluster_labels.sqlite", &db) == SQLITE_OK) {
        ...
        if (sqlite3_bind_int(stmt, 1, cluster) != SQLITE_OK || sqlite3_step(stmt) != SQLITE_ROW) {
            server_send(client, "500 No label.\n");
            return;
        }
        string label = string(reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0)));
        server_send(client, "BEGIN\n");
...
void do_command(char *command_c, int client)
{
    ...
    if (tokens[0] == "SHORTEST_PATH_A2A") {
        ...
        unordered_set<string> tx_hashes = a2c(tokens[1], (unsigned int) stoi(tokens[2]));
        ...
        unordered_set<string> tx_hashes = c2a((unsigned int) stoi(tokens[1]), tokens[2]);
        ...
        unordered_set<string> tx_hashes = c2c((unsigned int) stoi(tokens[1]), (unsigned int) stoi(tokens[2]));

...
impl BlockChain {
    ...
    fn walk_slice<'a, V: BlockChainVisitor<'a>>(&'a self, mut slice: &'a [u8], goal_prev_hash: &mut Hash, last_block: &mut Option<Block<'a>>, height: &mut u64, skipped: &mut HashMap<Hash, Block<'a>>, output_items: &mut HashMap<Hash, VecMap<V::OutputItem>>, visitor: &mut V) -> ParseResult<()> {
        while slice.len() > 0 {
            if skipped.contains_key(goal_prev_hash) {
                last_block.unwrap().walk(visitor, *height, output_items)?;
                debug!("(rewind - pre-step) Block {} - {} -> {}", height, last_block.unwrap().header().prev_hash(), last_block.unwrap().header().cur_hash());
                *height += 1;
                while let Some(block) = skipped.remove(goal_prev_hash) {
                    block.walk(visitor, *height, output_items)?;
                    debug!("(rewind) Block {} - {} -> {}", height, block.header().prev_hash(), block.header().cur_hash());
                    *height += 1;
            ...
            if block.header().prev_hash() != goal_prev_hash {
                ...
                if last_block.is_some() && block.header().prev_hash() == last_block.unwrap().header().prev_hash() {
                    debug!("Chain split detected: {} <-> {}. Detecting main chain and orphan.", last_block.unwrap().header().cur_hash(), block.header().cur_hash());
                    ...
                    loop {
                        ...
                        if block.header().prev_hash() == &first_orphan.header().cur_hash() {
                            // First wins
                            debug!("Chain split: {} is on the main chain!", first_orphan.header().cur_hash());
                            break;
                        }
                        if block.header().prev_hash() == &second_orphan.header().cur_hash() {
                            // Second wins
                            debug!("Chain split: {} is on the main chain!", second_orphan.header().cur_hash());
            ...
            if let Some(last_block) = *last_block {
                ...
                debug!("(last_block) Block {} - {} -> {}", height, last_block.header().prev_hash(), last_block.header().cur_hash());
                *height += 1;
            }
    ...
    pub fn walk<'a, V: BlockChainVisitor<'a>>(&'a self, visitor: &mut V) -> ParseResult<(u64, Hash, HashMap<Hash, VecMap<V::OutputItem>>)> {
        ...
        for (n, map) in self.maps.iter().enumerate() {
            info!("Parsing the blockchain: block file {}/{}...", n, self.maps.len() - 1);
            self.walk_slice(map, &mut goal_prev_hash, &mut last_block, &mut height, &mut skipped, &mut output_items, visitor)?;
        }

...
$title = "BitIodine - Find successors";
...
if ($show_form) {
    $section_show->appendChild(<h1>Get successors.</h1>);
    $section_show->appendChild(<p>Find out <strong>addresses</strong> that received Bitcoin from a particular <strong>address</strong>.</p>);
    $section_show->appendChild(
        <form>
            <input type="text" class="center" id="successors_address" placeholder="1AA2MKdGEv7kQZq2KXC5HdQcVaaCS8QcGE" value="1AA2MKdGEv7kQZq2KXC5HdQcVaaCS8QcGE" />
            <br class="clear" />
            <button class="button" id="successors_button">&rarr;฿</button>
        </form>);
} else {
    if (isset($error_message)) {
        $header_message = "No successor found :(";
        $subheader = $results = <span />;
        $description_or_error = <p><span class="error">{$error_message}</span></p>;
    } else {
        $header_message = "Here are the successors.";
        $subheader = <p>
                        <strong>{$cluster_size} address{$plural_form}</strong> received Bitcoin from <strong>{$address}</strong>.
                    </p>;
